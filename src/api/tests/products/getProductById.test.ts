...
test.describe('[API] [Products] [Get the Product by Id]', async function () {
  ...
  test('1GE-API] Should GET the product by valid Id ', async function ({
    ...
    const response = await productsAPIController.getById(token, productId);
    validateResponse(response, STATUS_CODES.OK, true, null);
    validateJsonSchema(oneProductResponseSchema, response);
  });

  test('2GE-API] Should NOT GET the product by invalid Id ', async function ({
    productsAPIController
  }) {
  ...
  test('3GE-API] Should NOT GET the product without Id ', async function ({
    productsAPIController
  }) {
